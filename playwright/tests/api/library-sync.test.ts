...
import { libraryManifestSchema, type SyncGameListCommand } from "@playnite-insights/lib/client";
...
test("add, update and delete a game on library sync", async ({ request }) => {
	...
	expect(manifest.gamesInLibrary.find((g) => g.gameId === addGame.Id)).toBeTruthy();
	expect(manifest.gamesInLibrary.find((g) => g.contentHash === addGame.ContentHash)).toBeTruthy();
	// Update
	...
	expect(manifest.gamesInLibrary.find((g) => g.gameId === updateGame.Id)).toBeTruthy();
	expect(
		manifest.gamesInLibrary.find((g) => g.contentHash === updateGame.ContentHash),
	).toBeTruthy();
	// Remove
