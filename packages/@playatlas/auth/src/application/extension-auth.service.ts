import type { ILogServicePort, ISignatureServicePort } from "@playatlas/common/application";
...
export const makeExtensionAuthService = ({
	...
	const verify: IExtensionAuthServicePort["verify"] = async ({ request }) => {
		...
		if (["POST", "PUT"].includes(request.method)) {
			...
			if (contentType.includes("application/json")) {
				...
				const canonicalDigest = createHash("sha256").update(requestBody, "utf-8").digest();
