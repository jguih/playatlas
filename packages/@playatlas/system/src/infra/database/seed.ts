...
export const seedDb = (db: DatabaseSync, logService: LogService) => {
  ...
  `;
	...
	const gamePlatformData = [
		...gameData.map((data) => {
			return [data[0], platformData[Math.floor(Math.random() * platformData.length)][0]];
		}),
	];
  ...
  `;
	...
	const gameGenreData = [
		...gameData.map((data) => {
			return [data[0], genreData[Math.floor(Math.random() * genreData.length)][0]];
		}),
	];
  ...
  `;
	...
	const gameDeveloperData = [
		...gameData.map((data) => {
			return [data[0], companyData[Math.floor(Math.random() * companyData.length)][0]];
		}),
	];
