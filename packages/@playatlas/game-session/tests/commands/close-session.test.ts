...
describe("Close Game Session Service", () => {
	...
	it("create new closed session", () => {
		...
		const command: CloseGameSessionCommand = makeCloseGameSessionCommand(requestDto);
		...
		// Assert
		expect(() => closeGameSessionRequestDtoSchema.parse(requestDto)).not.toThrow();
	...
	it.each([{ duration: -30 }, { duration: -4000 }, { duration: -1 }])(
		...
		({ duration }) => {
			...
			// Act & Assert
			expect(() => closeGameSessionRequestDtoSchema.parse(requestDto)).not.toThrow();
			expect(() => service.execute(command)).toThrowError(InvalidGameSessionDurationError);
	...
	it("throws when attempting to close an already closed session", () => {
		...
		// Act & Assert
		expect(() => closeGameSessionRequestDtoSchema.parse(requestDto)).not.toThrow();
		expect(() => service.execute(command)).toThrowError(GameSessionNotInProgressError);
		expect(deps.gameSessionRepository.update).not.toHaveBeenCalled();
	});
});
