...
export const makeCompletionStatusRepository = (
	...
	const upsertMany: CompletionStatusRepository["upsertMany"] = (completionStatuses) => {
		return repositoryCall(
			logService,
			() => {
				const db = getDb();
	...
	const all: CompletionStatusRepository["all"] = () => {
		return repositoryCall(
			...
			() => {
				...
				const completionStatuses = z.optional(z.array(completionStatusSchema)).parse(result);
				logService.debug(`Found ${completionStatuses?.length ?? 0} completion status(es)`);
