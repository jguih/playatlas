...
import { type GameNote, type GameNoteFilters, gameNoteSchema } from "@playnite-insights/lib/client";
import z from "zod";
...
export const makeGameNoteRepository = ({
	...
	const reconcileFromSource: GameNoteRepository["reconcileFromSource"] = (notes) => {
		return repositoryCall(
			logService,
			() => {
				if (notes.length === 0) return;
