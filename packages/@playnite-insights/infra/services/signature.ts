...
export const makeSignatureService = (deps: SignatureServiceDeps): SignatureService => {
	...
	const generateKeyPairAsync: SignatureService["generateKeyPairAsync"] = async () => {
		...
		try {
			...
			logService.info(`Skipping asymmetric key generation, both keys already exist`);
			return;
		} catch {
	...
	const verifyExtensionSignature: SignatureService["verifyExtensionSignature"] = ({
		...
		const isValid = verify.verify(registration.PublicKey, Buffer.from(signatureBase64, "base64"));
