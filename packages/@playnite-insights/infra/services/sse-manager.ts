		...
		this.stream = new ReadableStream({
			start: (controller) => {
				...
				if (!isAuthorized) {
					...
					this.logService.warning(`Request to create new Event Stream not authorized`);
					this.cleanup();
					return;
				}
...
export class SSEManager {
	...
	createStream({ isAuthorized, onCancel }: Omit<EventStreamDeps, "logService">): EventStream {
