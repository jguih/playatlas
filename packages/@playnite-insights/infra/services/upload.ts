...
export const makeUploadService = (deps: Partial<UploadServiceDeps>): UploadService => {
	const {
		streamUtilsService,
		logService,
		fileSystemService,
	...
	const uploadImagesAsync: UploadService["uploadImagesAsync"] = async (request, path, source) => {
		if (!request.body) {
		...
		bb.on("file", async (_, fileStream, { filename, mimeType }) => {
			...
			if (!allowedExts.includes(ext) || !mimeType.startsWith("image/")) {
				logService.warning(`Image upload rejected due to incorrect extension ${ext} or mimeType`);
				fileStream.resume();
				return;
			}
				...
				writeStream.on("finish", async () => {
					try {
						...
						if (existing) {
							...
							logService.info(`Image file already exists: ${existing.FilePath}`);
		...
		await new Promise((resolve, reject) => {
			streamUtilsService.readableFromWeb(request.body as ReadableStream).pipe(bb);
		...
		return Promise.all(filePromises).then((paths) => {
			...
			logService.info(
				`Upload image finished in ${duration.toFixed(1)}ms, ${uploadCount} image(s) were processed`,
			);
			return paths;
		});
	};
