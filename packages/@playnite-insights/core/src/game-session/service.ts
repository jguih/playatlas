...
import { type GameSessionService, type GameSessionServiceDeps } from "./service.types";
...
export const makeGameSessionService = ({
	...
	const open: GameSessionService["open"] = (command) => {
		...
		if (!existingGame) {
			logService.warning(`Attempted to open session for non existent game, skipping`);
			return false;
		}
		...
		if (result) {
			logService.info(`Created open session ${command.SessionId} for ${existingGame.Name}`);
		}
		return result;
	};
	...
	const close: GameSessionService["close"] = (command) => {
		...
		if (existing) {
			if (_isValidClosedSession(existing)) {
				logService.warning(`Attempted to close already closed session, skipping`);
				return false;
			}
		...
		if (_isValidCloseCommand(command) && existingGame) {
			...
			const result = gameSessionRepository.add(newSession);
			if (result) {
				logService.info(`Created closed session ${command.SessionId} for ${existingGame.Name}`);
			}
			return result;
		}
		...
		if (_isValidStaleCommand(command) && existingGame) {
			...
			const result = gameSessionRepository.add(newSession);
			if (result) {
				logService.info(`Created stale session ${command.SessionId} for ${existingGame.Name}`);
			}
			return result;
		}

		logService.warning(`Attempted to close session with invalid command or for non existent game`);
		return false;
	};
