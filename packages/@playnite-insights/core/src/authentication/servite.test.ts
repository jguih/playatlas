...
describe("Authentication service", () => {
	...
	it.each([
		...
		(headers) => {
			...
			expect(deps.extensionRegistrationRepository.getByExtensionId).not.toHaveBeenCalled();
			expect(deps.signatureService.verifyExtensionSignature).not.toHaveBeenCalled();
		},
	);
	...
	it("when verifying extension authorization, authorize with valid headers", () => {
		...
		expect(deps.extensionRegistrationRepository.getByRegistrationId).toHaveBeenCalledOnce();
		expect(deps.signatureService.verifyExtensionSignature).toHaveBeenCalledOnce();
	});
	...
	it("on POST or PUT, reject when X-ContentHash header is null", () => {
		...
		// Assert
		expect(isAuthorized).toBeFalsy();
		expect(deps.extensionRegistrationRepository.getByRegistrationId).not.toHaveBeenCalledOnce();
		expect(deps.signatureService.verifyExtensionSignature).not.toHaveBeenCalledOnce();
	});
});
