...
export const makeAuthService = ({
	...
	const verifyExtensionAuth: AuthService["verifyExtensionAuth"] = ({
		...
		logService.info(`${requestDescription}: Request authorized for ${extensionDescription}`);
		return true;
	};
	...
	const verifySessionId: AuthService["verifySessionId"] = ({ sessionId }) => {
		if (!sessionId) {
			return {
				...
				message: "Request rejected for instance due to invalid Authorization header",
			};
		}
	...
	const verifyInstanceAuth: AuthService["verifyInstanceAuth"] = ({ headers, request, url }) => {
		const requestDescription = `${request.method} ${url.pathname}`;
		const authorization =
	...
	const registerInstanceAsync: AuthService["registerInstanceAsync"] = async (password) => {
		...
		const { hash, salt } = await cryptographyService.hashPasswordAsync(password);
		const now = new Date();
	...
	const loginInstanceAsync: AuthService["loginInstanceAsync"] = async (password) => {
		const existing = instanceAuthenticationRepository.get();
		if (!existing)
			throw new ApiError(
		...
		if (!isValid)
			throw new ApiError({ error: { code: "not_authorized" } }, "Invalid credentials", 403);
		const now = new Date();
