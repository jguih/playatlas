...
export const makeMediaFilesService = ({
	...
	const checkIfImageExists = async (imagePath: string): Promise<boolean> => {
		try {
			await fileSystemService.access(imagePath, fileSystemService.constants.F_OK);
			return true;
		} catch {
			return false;
		}
	...
	const isValidUploadSource = (uploader: string | null): uploader is Image["Source"] => {
	...
	const uploadScreenshotsAsync: MediaFilesService["uploadScreenshotsAsync"] = async (request) => {
		const uploader = request.headers.get("X-Upload-Source");
		const source: Image["Source"] = isValidUploadSource(uploader) ? uploader : "unknown";
		logService.info(`Received request to upload images from ${source}`);
		return await uploadService.uploadImagesAsync(request, SCREENSHOTS_DIR, source);
	};

	const getAvailableScreenshots: MediaFilesService["getAvailableScreenshots"] = async () => {
