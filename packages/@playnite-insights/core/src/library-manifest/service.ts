import { ApiError, type PlayniteLibraryManifest } from "@playnite-insights/lib/client";
import { join } from "path";
import type { LibraryManifestService, LibraryManifestServiceDeps } from "./service.types";
...
export const makeLibraryManifestService = ({
	...
	const write: LibraryManifestService["write"] = async () => {
		...
		try {
			...
			if (!gamesManifestData) {
				logService.warning(`Library manifest was not written due to missing library data`);
				return;
			}
			...
			for (const folder of libraryFolders) {
				...
				const contentHash = await fileSystemService.readfile(contentHashFilePath, "utf-8");
				mediaExistsFor.push({
					gameId: folder,
			...
			await fileSystemService.writeFile(LIBRARY_MANIFEST_FILE, JSON.stringify(manifest, null, 2));
	...
	const get = async () => {
		try {
			const content = await fileSystemService.readfile(LIBRARY_MANIFEST_FILE, "utf-8");
			const asJson = JSON.parse(content.toString()) as PlayniteLibraryManifest;
			logService.debug(`Read and parsed library manifest file at ${LIBRARY_MANIFEST_FILE}`);
			return asJson ?? null;
		} catch (error) {
