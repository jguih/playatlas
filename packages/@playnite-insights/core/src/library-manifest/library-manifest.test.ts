...
describe("Library manifest", () => {
	...
	it("reads media files directory when manifest data is found", async () => {
		...
		// Assert
		expect(deps.fileSystemService.readdir).toHaveBeenCalledWith(deps.FILES_DIR, expect.anything());
	});
	...
	it("should write manifest file", async () => {
		...
		const [calledPath, calledContent] = deps.fileSystemService.writeFile.mock.calls[0];
