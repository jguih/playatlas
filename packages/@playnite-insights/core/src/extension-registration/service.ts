import { ApiError, type ExtensionRegistration } from "@playnite-insights/lib/client";
import type {
...
export const makeExtensionRegistrationService = ({
	...
	const register: ExtensionRegistrationService["register"] = (command) => {
		const existing = extensionRegistrationRepository.getByExtensionId(command.ExtensionId);
	...
	const revoke: ExtensionRegistrationService["revoke"] = (registrationId) => {
		const existing = extensionRegistrationRepository.getByRegistrationId(registrationId);
		if (!existing)
			throw new ApiError({ error: { code: "not_found" } }, "Extension registration not found", 404);
