...
export const makePlayniteLibraryImporterService = ({
	...
	const _fromDtoToPlayniteGame = (game: IncomingPlayniteGameDTO): PlayniteGame => {
		return {
			Id: game.Id,
	...
	const sync: PlayniteLibraryImporterService["sync"] = async (data) => {
		...
		try {
			...
			const uniquePlatforms = Array.from(new Map(platforms.map((p) => [p.Id, p])).values());
			uniquePlatforms.length > 0 && platformRepository.upsertMany(uniquePlatforms);
			...
			uniqueCompanies.length > 0 && companyRepository.upsertMany(uniqueCompanies);
			...
			const gamesToUpsert: PlayniteGame[] = addedOrUpdated.map(_fromDtoToPlayniteGame);
			gamesToUpsert.length > 0 && playniteGameRepository.upsertMany(gamesToUpsert);

			const gameGenresMap = new Map(
				addedOrUpdated.map((g) => [g.Id, g.Genres?.map((g) => g.Id) ?? []]),
			);
			gameGenresMap.size > 0 && playniteGameRepository.updateManyGenres(gameGenresMap);

			const gamePublishersMap = new Map(
				addedOrUpdated.map((g) => [g.Id, g.Publishers?.map((p) => p.Id) ?? []]),
			);
			gamePublishersMap.size > 0 && playniteGameRepository.updateManyPublishers(gamePublishersMap);

			const gameDevelopersMap = new Map(
				addedOrUpdated.map((g) => [g.Id, g.Developers?.map((d) => d.Id) ?? []]),
			);
			gameDevelopersMap.size > 0 && playniteGameRepository.updateManyDevelopers(gameDevelopersMap);

			const gamePlatformsMap = new Map(
				addedOrUpdated.map((g) => [g.Id, g.Platforms?.map((p) => p.Id) ?? []]),
			);
			gamePlatformsMap.size > 0 && playniteGameRepository.updateManyPlatforms(gamePlatformsMap);
			...
			if (data.RemovedItems.length > 0) {
				...
				for (const gameId of data.RemovedItems) {
					...
					try {
						...
						logService.error(`Failed to delete media folder ${gameMediaFolderDir}`, error as Error);
					}
				}
				const startDeleteMediaFoldersDuration = performance.now() - startDeleteMediaFolders;
				logService.debug(
			...
			if (totalGamesToChange > 0) {
				...
				const visibleTotalPlaytime = playniteGameRepository.getTotalPlaytimeSeconds({
					hidden: false,
				});
	 ...
	 */
	const importMediaFiles: PlayniteLibraryImporterService["importMediaFiles"] = async (
		request,
		url,
	) => {
		return new Promise(async (resolve, reject) => {
			...
			const stream = streamUtilsService.readableFromWeb(request.body! as unknown as ReadableStream);
			...
			bb.on("file", async (_, fileStream, { filename }) => {
				...
				const filePromise = new Promise<string>((resolve, reject) => {
					const writeStream = streamUtilsService.createWriteStream(tmpFilePath);
					writeStream.on("finish", () => {
						logService.debug(`Saved file ${tmpFilePath} to disk`);
						resolve(tmpFilePath);
					});
					writeStream.on("error", (error) => {
						logService.error(`Failed to save file ${tmpFilePath} to disk`, error);
						reject(error);
					});
					const chunks: Buffer[] = [];
			...
			bb.on("finish", async () => {
				try {
					...
					if (ctx.uploadCount === 0)
						throw new ApiError({ error: { code: "bad_request" } }, "No images uploaded", 400);
					if (!ctx.gameId)
						throw new ApiError({ error: { code: "bad_request" } }, "Missing gameId", 400);
					if (!ctx.mediaFilesHash)
						throw new ApiError({ error: { code: "bad_request" } }, "Missing media file hash", 400);

					ctx.game = playniteGameRepository.getById(ctx.gameId) ?? null;
					if (!ctx.game)
						throw new ApiError({ error: { code: "not_found" } }, "Game not found", 404);
					...
					await fileSystemService.writeFile(contentHashPath, ctx.mediaFilesHash, "utf-8");
					...
					logService.debug(`Moved all files in ${ctx.tmpDir} to ${uploadDir}`);
