...
export class GameFactory {
	private buildGame = ({ ...props }: Partial<FullGame> = {}): FullGame => {
		return {
			...
			Developers: props?.Developers ?? Array.from({ length: 5 }, () => faker.company.name()),
			Publishers: props?.Publishers ?? Array.from({ length: 5 }, () => faker.company.name()),
			Genres: props?.Genres ?? [
				...faker.helpers.arrayElements(["Action", "RPG", "Strategy", "Adventure"]),
			],
			Platforms: props?.Platforms ?? [faker.helpers.arrayElement(["PC", "Linux", "Xbox", "PS"])],
	...
	getGames = (count: number, { ...props }: Partial<FullGame> = {}): FullGame[] => {
		const games = Array.from({ length: count }, () => this.buildGame({ ...props }));
		return games;
	};
}
